...
def set_stack_name():
    ...
    The env.tag dictates which randomly generated suffix
    tag='active' should be saved as key to switch stacks.
    ...
    if not hasattr(env, 'tag'):
        stack_tag = cfn_config.stack_id
    else:
        stack_tag = env.tag
    if env.tag=='active':
        raise CfnConfigError("The value of tag cannot be 'active'")
    env.tag = stack_tag
    legacy_name = "{0}-{1}".format(env.application, env.environment)
    # get_config needs a stack_name so this is a hack because we don't
    # know it yet...
    ...
    stack_suffix = cfn_config.stack_id
